...
from optims import adashift
from optims import adabound
from optims import nosadam
from optims import lookahead
from optims import padam
from optims import padam_star
...
if __name__ == '__main__':
    ...
    parser.add_argument('--alpha', default=0.125, type=float, help='learning rate for lookahead')
    parser.add_argument('--gamma_nos', default = 0, type=float, help=' gamma for nosadam')
    parser.add_argument('--partial', default=0.125, type=float,help='partial adaptive parameter')
    ...
    parser.add_argument('--lr_decay_epoch', default=150, type = int, help='The epoch that learning rate to decay')
    ...
    # model parameters
    parser.add_argument('--model', '-m', default='vgg9',help='vgg9')
    ...
    if args.optimizer == 'sgd':
        ...
        optimizer = adabound.AdaBound(net.parameters(), lr=args.lr, final_lr=args.final_lr, gamma=args.gamma,
                                      weight_decay=args.weight_decay)
    elif args.optimizer == 'amsbound':
        optimizer = adabound.AdaBound(net.parameters(), lr=args.lr, final_lr=args.final_lr, gamma=args.gamma,
                                      weight_decay=args.weight_decay, amsbound=True)
    elif args.optimizer == 'adashift':
        optimizer = adashift.AdaShift(net.parameters(), lr=args.lr)
    elif args.optimizer == 'swats':
        optimizer = swats.SWATS(net.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        ...
        optimizer = nosadam.NosAdam(net.parameters(),lr =args.lr, weight_decay= args.weight_decay, gamma = args.gamma_nos)
    elif args.optimizer == 'lookahead':
        base_opt = optim.Adam(net.parameters(), lr=args.lr, betas=(0.9, 0.999)) # Any optimizer
        optimizer = lookahead.Lookahead(base_opt, k=5, alpha=args.alpha) # Initialize Lookahead
    elif args.optimizer == 'padam':
        optimizer = padam.Padam(net.parameters(), lr=args.lr, betas=(0.9, 0.999),weight_decay=args.weight_decay, partial=args.partial)
    elif args.optimizer == 'padam_star':
        optimizer = padam_star.PadamStar(net.parameters(), lr = args.lr, betas=(0.9, 0.999),weight_decay=args.weight_decay,gamma = args.gamma_nos )
    elif args.optimizer == 'nospadam':
        optimizer = nospadam.NosPadam(net.parameters(), lr = args.lr, betas = (0.9, 0.999), weight_decay = args.weight_decay, gamma = args.gamma_nos)
    elif args.optimizer == 'nosadashift':
        optimizer = nosadashift.NosAdaShift(net.parameters(), lr = args.lr, betas= (0.9, 0.999), weight_decay=args.weight_decay, gamma = args.gamma_nos)
    else:
    ...
    if not args.resume_model:
        ...
        train_accuracies.append(100 - train_err)
        test_accuracies.append(100 - test_err)
        ...
        torch.save({'train_acc': train_accuracies, 'test_acc': test_accuracies}, 'curve/' + save_folder)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if int(epoch) == args.lr_decay_epoch:
            lr *= args.lr_decay
