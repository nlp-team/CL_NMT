...
'''
    NosAdam + Padam
'''
class NosPadam(Optimizer):

    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, gamma=0, lr_decay=False, partial=1 / 4):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, gamma=gamma, lr_decay=lr_decay, partial = partial)
        super(NosPadam, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(NosPadam, self).__setstate__(state)
