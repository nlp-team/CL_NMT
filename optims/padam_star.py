...
class PadamStar(Optimizer):
    ...
    def __init__(self, params, lr=1e-1, betas=(0.9, 0.999), eps=1e-8, weight_decay=0, amsgrad=True, gamma=0):
        ...
        super(PadamStar, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if not state['be_sgd']:
                    partial = 0.5 * (1 - state['B_old'] / state['B_new'])
                    state['B_old'] += math.pow(state['step'], -group['gamma'])
                    state['B_new'] += math.pow(state['step'] + 1, -group['gamma'])
                    if partial <= 1e-6:
                        state['be_sgd'] = True
                else:
                    partial = 0
