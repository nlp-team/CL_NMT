...
def project_othermodels_trajectory(dir_file, w, model_files, ignore_embedding, proj_method='cos'):
    """
        Project the optimization trajectory onto the given two directions.
    ...
    for model_file in model_files:
        state = torch.load(model_file,
                           map_location=lambda s, l: torch.serialization.default_restore_location(s, 'cpu'), )
        args2 = state['args']
...
def setup_othermodels_PCA_directions(args, ignore_embedding, model_files, w):
    ...
    for model_file in model_files:
        print(model_file)
        state = torch.load(model_file, map_location=lambda s, l: torch.serialization.default_restore_location(s, 'cpu'),)
        args2 = state['args']
        args2.data = 'fairseq_master/'+args2.data
#        task = tasks.setup_task(args2)
#        model2 = task.build_model(args2)
